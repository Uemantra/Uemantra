...
func refresh() -> void:
	_clear_list()
	var fragment_ids := _collect_fragment_entry_ids()
	if fragment_ids.is_empty():
		var empty_label := Label.new()
		empty_label.text = "No recovered fragments yet."
		empty_label.add_theme_color_override("font_color", Color(0.5, 0.45, 0.4))
		entry_list.add_child(empty_label)
		return
	for entry_id in fragment_ids:
		_add_entry_row(entry_id)


func _collect_fragment_entry_ids() -> Array:
	# Fragments are cache-derived entries whose catalogue_section is not
	# one of the specialised sections.
	var ids: Array = []
	for entry_id in GameState.catalogue_entries:
		if entry_id in GameState.companions:
			continue
		if not DataManager.get_relic(entry_id).is_empty():
			continue
		var yield_data := _find_cache_yield(entry_id)
		if yield_data.is_empty():
			continue
		var section_tag: String = yield_data.get("catalogue_section", "")
		if section_tag not in ["people_we_never_knew", "what_the_machines_said", "relics"]:
			ids.append(entry_id)
	return ids
...
func _add_entry_row(entry_id: String) -> void:
	...
	var first_line: String = content.split("\n")[0] if content != "" else ""
	if first_line.length() > 80:
		first_line = first_line.substr(0, 77) + "..."
